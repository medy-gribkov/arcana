...
export async function optimizeInteractive(): Promise<void> {
  ...
  lines.push("");
  lines.push(
    `${chalk.bold(String(budget.count))} skills installed, ${chalk.bold(String(budget.totalKB))} KB total (~${(budget.totalTokens / 1000).toFixed(0)}K tokens)`,
  );
