...
function writeTeamConfig(config: TeamConfig): void {
...
function teamInit(opts: { json?: boolean }): void {
  const configPath = getTeamConfigPath();
...
async function teamSync(opts: { json?: boolean }): Promise<void> {
...
function teamAdd(skill: string, opts: { json?: boolean }): void {
  try {
  ...
  const entry: TeamSkillEntry = { name: skill };
...
function teamRemove(skill: string, opts: { json?: boolean }): void {
  const config = readTeamConfig();
  if (!config) {
    if (opts.json) {
      console.log(JSON.stringify({ error: "No team config found. Run: arcana team init" }));
...
function teamList(opts: { json?: boolean }): void {
  const config = readTeamConfig();
  if (!config) {
    if (opts.json) {
      console.log(JSON.stringify({ error: "No team config found. Run: arcana team init" }));
