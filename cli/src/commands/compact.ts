...
function analyzeProject(projDir: string, projName: string): ProjectAnalysis {
  ...
  const totalBytes =
    mainFiles.reduce((s, f) => s + f.sizeBytes, 0) +
  ...
  const reclaimableBytes =
    agentFiles.reduce((s, f) => s + f.sizeBytes, 0) + sessionDirs.reduce((s, d) => s + d.sizeBytes, 0);
 ...
 */
function pruneMainSessions(
  analyses: ProjectAnalysis[],
  projectsDir: string,
  pruneDays: number,
  dryRun: boolean,
  json: boolean,
): { pruned: { project: string; file: string; sizeMB: string; daysOld: number }[]; reclaimedBytes: number } {
  const pruned: { project: string; file: string; sizeMB: string; daysOld: number }[] = [];
...
export async function compactCommand(opts: {
  ...
  if (opts.json) {
    ...
    if (opts.prune) {
      ...
      if (!dryRun) {
        jsonResult.totalReclaimed = (jsonResult.totalReclaimed as number) + pruneResult.reclaimedBytes;
      }
    }
  ...
  for (const a of analyses) {
    ...
    console.log(
      `    ${ui.success("Keep:")} ${a.mainFiles.length} main session${a.mainFiles.length !== 1 ? "s" : ""} ${ui.dim(`(${formatBytes(mainSize)})`)}`,
    );
    ...
    if (a.agentFiles.length > 0) {
      ...
      console.log(
        `    ${ui.warn("Remove:")} ${a.agentFiles.length} agent log${a.agentFiles.length !== 1 ? "s" : ""} ${ui.dim(`(${formatBytes(agentSize)})`)}`,
      );
    ...
    if (a.sessionDirs.length > 0) {
      ...
      if (dirSize > 0) {
        console.log(
          `    ${ui.warn("Remove:")} ${a.sessionDirs.length} session dir${a.sessionDirs.length !== 1 ? "s" : ""} ${ui.dim(`(${formatBytes(dirSize)})`)}`,
        );
